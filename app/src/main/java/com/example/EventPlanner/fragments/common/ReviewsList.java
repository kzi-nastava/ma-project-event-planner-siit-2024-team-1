package com.example.EventPlanner.fragments.common;

import android.os.Bundle;

import androidx.fragment.app.Fragment;
import androidx.lifecycle.ViewModelProvider;
import androidx.recyclerview.widget.LinearLayoutManager;
import androidx.recyclerview.widget.RecyclerView;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.example.EventPlanner.R;
import com.example.EventPlanner.adapters.common.ReviewOverviewAdapter;
import com.example.EventPlanner.adapters.event.EventOverviewAdapter;
import com.example.EventPlanner.databinding.FragmentReviewsListBinding;
import com.example.EventPlanner.fragments.event.EventListViewModel;

/**
 * A simple {@link Fragment} subclass.
 * Use the {@link ReviewsList#newInstance} factory method to
 * create an instance of this fragment.
 */
public class ReviewsList extends Fragment {

    // TODO: Rename parameter arguments, choose names that match
    // the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
    private static final String ARG_PARAM1 = "param1";
    private static final String ARG_PARAM2 = "param2";

    // TODO: Rename and change types of parameters
    private String mParam1;
    private String mParam2;

    private FragmentReviewsListBinding reviewsListBinding;
    private ReviewsListViewModel reviewsListViewModel;

    public ReviewsList() {
        // Required empty public constructor
    }

    /**
     * Use this factory method to create a new instance of
     * this fragment using the provided parameters.
     *
     * @param param1 Parameter 1.
     * @param param2 Parameter 2.
     * @return A new instance of fragment ReviewsList.
     */
    // TODO: Rename and change types and number of parameters
    public static ReviewsList newInstance(String param1, String param2) {
        ReviewsList fragment = new ReviewsList();
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, param1);
        args.putString(ARG_PARAM2, param2);
        fragment.setArguments(args);
        return fragment;
    }

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {
            mParam1 = getArguments().getString(ARG_PARAM1);
            mParam2 = getArguments().getString(ARG_PARAM2);
        }
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        reviewsListBinding=FragmentReviewsListBinding.inflate(getLayoutInflater());
        RecyclerView reviewRecyclerView=reviewsListBinding.reviewsRecyclerView;
        reviewRecyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        reviewsListViewModel=new ViewModelProvider(requireActivity()).get(ReviewsListViewModel.class);
        reviewsListViewModel.getReviews().observe(getViewLifecycleOwner(), reviews->{
            ReviewOverviewAdapter reviewsAdapter = new ReviewOverviewAdapter(requireActivity(), reviews,reviewsListViewModel);
            reviewRecyclerView.setAdapter(reviewsAdapter);
            reviewsAdapter.notifyDataSetChanged();

        });
        reviewsListViewModel.getPendingReviews();

        return reviewsListBinding.getRoot();
    }
}